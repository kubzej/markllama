...
{#snippet modelRow(model: OllamaModel)}
	...
	>
		<button
			...
			class="flex min-w-0 flex-1 items-start gap-2 rounded-lg px-2.5 py-1.5 text-left transition-colors duration-150 hover:bg-[var(--control-hover)]"
		>
			<span class="block min-w-0 flex-1">
				<span class="flex min-w-0 items-baseline gap-1.5">
					<span
						class="min-w-0 truncate text-sm font-medium {selected
					...
					{#if note.alias.trim()}
						<span class="min-w-0 truncate font-mono text-[11px] text-[var(--text-muted)]"
							>{model.name}</span
						>
					{/if}
					{#if numCtx}
						<span
							...
							class="shrink-0 rounded-full bg-[var(--surface-inset)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text-secondary)]"
						>
				...
				{#if note.description.trim()}
					<span class="model-selector-description mt-0.5 text-xs text-[var(--text-secondary)]"
		...
		<button
			...
			class="shrink-0 self-start rounded-md p-1.5 text-[var(--text-muted)] transition-colors duration-150 hover:bg-[var(--control-hover)] hover:text-[var(--text-primary)]"
		>
			<svg
				viewBox="0 0 24 24"
				class="size-3.5"
...
<div class="relative" bind:this={rootEl}>
	<button
		...
		aria-expanded={open}
		class="flex max-w-44 items-center gap-1 rounded-lg px-2.5 py-1.5 text-[var(--text-secondary)] transition-colors duration-150 hover:bg-[var(--control-hover)] hover:text-[var(--text-primary)] disabled:cursor-not-allowed disabled:opacity-40"
	>
		<span class="truncate">
	...
	{#if open}
		<div
			role="listbox"
			class="model-selector-popover app-popover absolute top-full right-0 z-[90] mt-1 overflow-y-auto rounded-xl p-1"
		>
