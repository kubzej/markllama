...
<aside class="app-surface flex w-64 shrink-0 flex-col overflow-hidden rounded-2xl">
	<div class="app-panel-header flex items-center justify-between gap-1 px-3 py-2.5">
		<span class="min-w-0 flex-1 truncate text-xs font-medium text-[var(--text-secondary)]">
			{rootName}
		</span>
		<button
			title="Refresh"
	...
	<div class="flex-1 overflow-y-auto p-1.5">
		{#if projectState.loading}
			<p class="px-2 py-1.5 text-xs text-[var(--text-muted)]">Scanning…</p>
		...
		{:else if projectState.tree && projectState.tree.kind === 'dir' && projectState.tree.children.length === 0}
			<p class="px-2 py-1.5 text-xs text-[var(--text-muted)]">No Markdown files in this folder.</p>
